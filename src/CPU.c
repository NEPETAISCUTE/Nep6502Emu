...
void CPUPutState(CPU* cpu) {
	...
	printf("S=$%02X\n", cpu->S);
	printf("PC=$%04X\n", cpu->PC);
	printf("(PC)=$%02X\n", MEMORY_GET_BYTE(cpu->RAM, cpu->PC));
	printf("P=");
	...
	printf("\n");
	if (!cpu->isOpcodeFetched)
		printf("need to fetch opcode to know how many bytes i need to fetch total for instruction\n");
	else
		printf("%d bytes left to fetch\n", cpu->totalFetchCycles - cpu->fetchCycles);
}
